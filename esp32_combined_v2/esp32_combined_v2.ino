/*
  ESP32 — Combined USB + BLE  [ VERSION 2.0 ]
  ─────────────────────────────────────────────
...
#define FW_VERSION    "2.0"   // <<< Firmware version reported to dashboard
...
void setup() {
  ...
  // Report version so dashboard can check for updates
  Serial.println("VERSION:" FW_VERSION);
  Serial.println("[v2.0] 🚀 ESP32 is now running firmware version 2.0 — connected via USB. Welcome to the upgrade!");
...
void loop() {
  ...
  if (bleConnected && !bleGreeted) {
    delay(300);
    bleSend("[v2.0] 🚀 ESP32 running firmware 2.0 — connected via Bluetooth. Upgrade successful!");
    bleGreeted = true;
  }
  if (!bleConnected) bleGreeted = false;

  // Heartbeat every 10 s on both channels
  if (millis() - lastHB > 10000) {
    lastHB = millis();
    Serial.println("[v2.0] heartbeat: esp32 usb alive — firmware 2.0");
    bleSend("[v2.0] heartbeat: esp32 ble alive — firmware 2.0");
  }
  ...
  if (Serial.available()) {
    ...
    msg.trim();
    // Dashboard sends VERSION? to query firmware
    if (msg == "VERSION?") {
      Serial.println("VERSION:" FW_VERSION);
    } else {
      Serial.print("echo: ");
