/*
  AlphaSense — Combined USB + BLE  [ VERSION 1.0 ]
  ─────────────────────────────────────────────────
  - USB Serial : always active, sends greeting on boot
  - BLE (NUS)  : advertises as "AlphaSense V1", sends greeting on connect
...
// ── HMAC-SHA256 helper ────────────────────────
// Returns lowercase hex string of HMAC-SHA256(key, message)
...
void handleCommand(const String& raw) {
  ...
  if (cmd == "VERSION?") {
    ...
    // Extract nonce and compute HMAC response
  ...
  // All other commands silently ignored — no arbitrary echo
}
...
void setup() {
  ...
  Serial.println("VERSION:" FW_VERSION);
  Serial.println("[v1.0] Hello from AlphaSense — connected via USB. This is firmware version 1.0.");
  ...
  Serial.println("BLE advertising as AlphaSense V1...");
}
...
void loop() {
  ...
  if (bleConnected && !bleGreeted) {
    delay(300);
    bleSend("[v1.0] Hello from AlphaSense — connected via Bluetooth. Firmware version 1.0.");
    bleGreeted = true;
  }
  ...
  if (millis() - lastHB > 10000) {
    lastHB = millis();
    Serial.println("[v1.0] heartbeat: AlphaSense usb alive — firmware 1.0");
    bleSend("[v1.0] heartbeat: AlphaSense ble alive — firmware 1.0");
  }
