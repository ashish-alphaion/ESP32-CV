/*
  ESP32 — Combined USB + BLE  [ VERSION 1.0 ]
  ─────────────────────────────────────────────
...
#define FW_VERSION    "1.0"   // <<< Firmware version reported to dashboard
...
void setup() {
  ...
  Serial.println("[v1.0] Hello from ESP32 — connected via USB. This is firmware version 1.0.");
...
void loop() {
  ...
  if (bleConnected && !bleGreeted) {
    delay(300);
    bleSend("[v1.0] Hello from ESP32 — connected via Bluetooth. Firmware version 1.0.");
    bleGreeted = true;
  }
  ...
  if (millis() - lastHB > 10000) {
    lastHB = millis();
    Serial.println("[v1.0] heartbeat: esp32 usb alive — firmware 1.0");
    bleSend("[v1.0] heartbeat: esp32 ble alive — firmware 1.0");
  }
